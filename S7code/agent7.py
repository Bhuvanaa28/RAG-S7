...
def main() -> None:
    console.print("[bold green]Welcome to the S7 Agent.[/bold green]")
    while True:
        try:
        ...
        try:            
            ...
            console.print(f"\n[bold magenta][S7 Agent] Final Answer:[/bold magenta]\n{answer}")
            console.print(f"{'═' * 78}\n")    
        except Exception as e:
            if hasattr(e, "exceptions"):
                console.print(f"\n[bold red][S7 Agent] Run failed with multiple errors:[/bold red]")
                for i, sub_e in enumerate(e.exceptions):
                    console.print(f"  [bold red]Error {i+1}:[/bold red] {sub_e}")
            else:
                console.print(f"\n[bold red][S7 Agent] Run failed: {e}[/bold red]")
