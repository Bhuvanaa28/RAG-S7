...
async def _crawl4ai_fetch(url: str, timeout: int = 60) -> dict:
    """Fetch a URL via crawl4ai running in an isolated subprocess.

    os.dup2 only redirects the C-runtime fd, not the Windows HANDLE that
    Playwright/Chromium subprocesses inherit. Running crawl4ai in a separate
    process with stdout=PIPE / stderr=DEVNULL fully isolates all browser noise
    from the MCP stdio JSON-RPC stream.
    """
    # Build a self-contained script; the subprocess prints only the JSON result
    # to its own stdout, which we capture. All crawl4ai / Playwright noise goes
    # to its stderr which we discard.
    script = (
        "import asyncio, json, sys\n"
        "from crawl4ai import AsyncWebCrawler\n"
        "\n"
        "async def main():\n"
        f"    async with AsyncWebCrawler(verbose=False) as crawler:\n"
        f"        r = await crawler.arun(url={url!r})\n"
        "    md = r.markdown\n"
        "    raw = (\n"
    ...
    proc = await asyncio.create_subprocess_exec(
        ...
        stderr=asyncio.subprocess.DEVNULL,  # silence all Playwright / crawl4ai noise
    ...
    try:
        ...
        raise RuntimeError(f"crawl4ai timed out after {timeout}s fetching {url!r}")

    if proc.returncode != 0:
        raise RuntimeError(
            f"crawl4ai subprocess exited {proc.returncode} while fetching {url!r}"
        )
