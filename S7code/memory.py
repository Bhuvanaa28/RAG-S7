...
def _try_embed(text: str, task_type: str) -> list[float] | None:
    ...
    try:
        ...
        print(f"[memory] embedding failed ({e!r}); item written without vector", file=sys.stderr)
        return None
...
def remember(
    ...
    *,
    ...
    try:
        ...
        print(f"[memory.remember] classifier failed ({e!r}); falling back to fact-write", file=sys.stderr)
